...
const isEngagementEmpty = (engagement) => {
	return Math.max(...[
		engagement['preWeak'], engagement['hack'], engagement['midWeak'],
		engagement['grow'], engagement['postWeak']
	]) == 0;
};
...
export async function main(ns) {
	...
	if (drainMode) {
		...
		ns.print('NoWin mode activated! w0r1d_d43m0n will be ignored, avoiding completing the BitNode.')
	}
	...
	while (true) {
		...
		for (let server of servers) {
			if (server == 'darkweb' || server.indexOf('hacknet-server') > -1) continue;  // Skip darkweb & hacknet servers
			...
			engagement = engagements.has(target) ? engagements.get(target) : null;
			...
			for (let [task, threadCount] of Object.entries(threads)) {
				...
				engagement = engagements.get(target);
				...
				if (task == 'time' || threadCount == 0) {
					...
					engagement['grow'] = 0;  // Prevent wasted grow cycles due to bad calculation
					continue;
				}
				...
				if (task == 'hack') {
					let delta = (actualThreads / threadCount) * engagement['hack'];
					engagement['hack'] = Math.max(engagement['hack'] - delta, 0);
